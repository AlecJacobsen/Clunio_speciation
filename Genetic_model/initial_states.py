...
for i in range(10000):
    ...
    val = np.array(res.stdout.decode('utf-8').split('\n')[-3])
    try:
        arr[i] = float(val)
    except ValueError:
        arr[i] = np.nan
    phenos = np.array(res.stdout.decode('utf-8').split('\n')[-2].split())
    phenos = np.array([float(x) for x in phenos])
    phenos_array[i,:] = np.histogram(phenos, bins = 30, range = (0,30))[0]

np.save('initial_states/%s_splitGens.npy' % args.X,arr)
np.save('initial_states/%s_Phenos.npy' % args.X,phenos_array)
